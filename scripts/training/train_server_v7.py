...
class Config:
    ...
    TRAIN_PATH      = os.path.join(BASE_DIR, 'data', 'splits', 'combined_train.txt')
    EVAL_PATH       = os.path.join(BASE_DIR, 'data', 'splits', 'rockyou_eval.txt')
    OUTPUT_PATH     = os.path.join(BASE_DIR, 'output', 'results', 'v7_results.json')
    GEN_PATH        = os.path.join(BASE_DIR, 'output', 'generated', 'v7_generated.txt')

    _FAST_DIR       = '/dev/shm/pwdgen_v7' if os.path.exists('/dev/shm') else os.path.join(BASE_DIR, 'output', 'models')
    MODEL_PATH      = os.path.join(_FAST_DIR, 'v7_model.pt')
    CHECKPOINT_PATH = os.path.join(_FAST_DIR, 'v7_checkpoint.pt')
...
class CharTokenizer:
    ...
    def encode_all(self, passwords, max_len=None):
        ...
        n = len(valid)
        print(f"   Encodage de {n:,} passwords...")
        t0 = time.time()
        result = np.full((n, max_len), pad, dtype=np.int16)
        for i, pwd in enumerate(valid):
            ...
            if (i + 1) % 2_000_000 == 0:
                print(f"   {i+1:,}/{n:,} ({(i+1)/n*100:.0f}%) en {time.time()-t0:.0f}s")
...
class CausalSelfAttention(nn.Module):
    def __init__(self, embed_dim, num_heads, dropout=0.1):
        ...
        self.rope = RotaryEmbedding(self.head_dim, max_seq_len=Config.MAX_SEQ_LEN + 8)
    ...
    while len(generated) < num_generate and total_attempts < max_attempts:
        ...
        if new_added < bs * 0.005:
            ...
            if stale_count >= stale_limit:
                print(f"      Stale ({stale_count}x): {len(generated):,} uniques")
                break
        else:
            stale_count = 0
...
def train_model(model, train_data, val_loader, eval_passwords, tokenizer, config,
    ...
    num_batches = train_data.size(0) // config.BATCH_SIZE if gpu_mode else len(train_data)
    ...
    print(f"   Mode: {'GPU-RESIDENT' if gpu_mode else 'DataLoader'}")
    if device == 'cuda':
        print(f"   GPU: {torch.cuda.get_device_name(0)} | VRAM: {torch.cuda.get_device_properties(0).total_memory//1024**3}GB")
    print(f"   Params: {model.count_parameters():,} | Archi: {config.NUM_LAYERS}L-{config.EMBED_DIM}D-{config.NUM_HEADS}H (RoPE)")
    print(f"   Epochs: {start_epoch+1} -> {config.EPOCHS} | Batch eff: {config.BATCH_SIZE*accum:,}")
    print(f"   Steps/epoch: {steps_per_epoch:,} | Early stop: {early_stopping.status()}")
    print(f"{'='*60}")

    NFS_DIR = os.path.join(config.BASE_DIR, 'output', 'models')
    ...
    for epoch in range(start_epoch, config.EPOCHS):
        ...
        if _interrupted:
            ...
            print(f"   Checkpoint sauvegarde (epoch {epoch+1})")
            break
        ...
        print(f"\n   +{'='*50}+")
        print(f"   | Epoch {epoch+1:3d}/{config.EPOCHS} | Temps: {elapsed:.0f}s ({elapsed/60:.1f}min)")
        ...
        if (epoch + 1) % 5 == 0 and eval_passwords:
            ...
            qg   = generate_passwords(model, tokenizer, 50_000, 0.7, 50, 0.92, device, 4096)
            es   = set(eval_passwords[:10_000])
            qm   = qg & es
        ...
        if (epoch + 1) % 3 == 0:
            ...
            if os.path.exists(config.MODEL_PATH):
                shutil.copy2(config.MODEL_PATH, os.path.join(NFS_DIR, 'v7_model.pt'))
            print(f"   Synchro NFS (epoch {epoch+1})")
...
def full_evaluation(model, tokenizer, eval_passwords, config):
    ...
    print(f"\n{'='*60}")
    print(f"EVALUATION COMPLETE — {len(eval_set):,} passwords eval")
    print(f"Temperatures: {config.TEMPERATURE}")
    print(f"{'='*60}")
    ...
    for temp in config.TEMPERATURE:
        ...
        print(f"   Coverage: {len(matches)}/{len(eval_set)} = {coverage:.2f}% en {gen_time:.0f}s")
        if matches:
    ...
    print(f"\n{'='*60}")
    print(f"COVERAGE COMBINE ({len(config.TEMPERATURE)} temps): {len(all_matches)}/{len(eval_set)} = {combined:.2f}%")
    print(f"Total unique genere: {len(all_generated):,}")
    print(f"{'='*60}")

    os.makedirs(os.path.dirname(config.GEN_PATH), exist_ok=True)
...
def main():
    ...
    NFS_MODEL = os.path.join(config.BASE_DIR, 'output', 'models', 'v7_model.pt')
    NFS_CKPT  = os.path.join(config.BASE_DIR, 'output', 'models', 'v7_checkpoint.pt')
    os.makedirs(config._FAST_DIR, exist_ok=True)

    if generate_only:
        if os.path.exists(NFS_MODEL):
            shutil.copy2(NFS_MODEL, config.MODEL_PATH)
            print(f"Copie modele NFS -> RAM disk (generate-only)")
        if os.path.exists(NFS_CKPT):
    ...
    def load_passwords(path, max_len=None):
        ...
        with open(path, 'r', encoding='utf-8', errors='ignore') as f:
            for line in f:
                pwd = line.strip()
                if pwd and not pwd.startswith('#') and 2 <= len(pwd) <= ml:
                    pwds.append(pwd)
        return pwds
    ...
    if not has_checkpoint:
        ...
        all_for_vocab = (all_train_pwds if all_train_pwds else []) + eval_pwds
        tokenizer.fit(all_for_vocab)
        model = PasswordTransformer(
            tokenizer.vocab_size, config.EMBED_DIM, config.NUM_HEADS,
            config.NUM_LAYERS, config.FF_DIM, config.MAX_SEQ_LEN, config.DROPOUT
        )
    ...
    with open(config.OUTPUT_PATH, 'w') as f:
        json.dump({'version': SCRIPT_VERSION, 'results': eval_results}, f, indent=2)
    print(f"\nResultats: {config.OUTPUT_PATH}")
